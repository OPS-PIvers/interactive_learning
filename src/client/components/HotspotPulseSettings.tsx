          ...
          <div key={hotspot.id} className="bg-slate-700 rounded p-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2">
                <div className={`w-4 h-4 rounded-full ${hotspot.color || 'bg-gray-500'}`}></div> {/* Added default color */}
                <span className="text-sm text-white">{hotspot.title}</span>
              </div>
              <label className="flex items-center space-x-1 cursor-pointer"> {/* Added cursor-pointer */}
                <input
                  type="checkbox"
                  checked={hotspot.defaultPulse === undefined ? true : hotspot.defaultPulse} // Default to true if undefined
